...
export const stats = [
  ...
  { value: "173", unit: "개", label: "직접 만들어 굴리는 저장소" },
...
export const layers = [
  ...
  },
  {
    ...
    body:
      "오래 다뤄 온 영역을 에이전트로 확장해 곧바로 구현으로 옮깁니다. 저장소 173개를 그렇게 만들었고 " +
...
export const work = [
  ...
  },
  {
    ...
    era: "2026",
    org: "ThakiCloud",
    title: "음악 생성 모델 적재부터 서빙까지, 실행 스택으로 12.7배",
    summary: "공개 이틀 된 57GB 음악 생성 모델을 사내 GPU에 올리고, 같은 카드에서 처리량을 12.7배로 끌어올렸습니다.",
    problem:
      "새 멀티모달 모델이 나오면 보통 '되네'에서 멈춥니다. 그런데 제품이 되려면 한 장의 GPU가 " +
      "시간당 몇 곡을 내는지, 곡 하나에 얼마가 드는지를 알아야 합니다. 그 숫자가 없으면 가격도 용량도 못 정합니다.",
    approach:
      "먼저 적재 경로를 실측했습니다. 공식 다운로드 클라이언트가 2.1MB/s인데 병렬 HTTP로 받으니 48MB/s였습니다. " +
      "받은 가중치를 내부 오브젝트 스토리지에 올려 두니 이후 모든 잡이 75초에 모델을 확보합니다. " +
      "그다음 같은 모델을 두 가지 실행 스택으로 돌려 비교했습니다. 레퍼런스 파이프라인과 서빙 엔진입니다.",
    result:
      "레퍼런스 경로가 시간당 36곡, 서빙 스택이 463곡이었습니다. 하드웨어도 가중치도 같고 실행 방식만 다릅니다. " +
      "곡당 원가는 약 $0.007이고, 5개 언어로 17곡을 실제로 만들었습니다. " +
      "모델 카드가 5분짜리 완곡을 낸다고 했지만 재현되지 않은 점도 같이 기록했습니다.",
    metrics: [
      { k: "실행 스택 차이", v: "36 → 463곡/시간" },
      { k: "곡당 원가", v: "약 $0.007" },
      { k: "적재 시간", v: "162분 → 75초" }
    ],
    stack: ["B200", "Kubernetes", "diffusers", "SGLang", "오브젝트 스토리지"],
    tags: ["멀티모달", "벤치마크", "인프라"],
    featured: true
  },
  {
    ...
    era: "2026",
    org: "개인 프로젝트 · 오픈소스",
    title: "파일을 서버에 올리지 않는 문서 변환기 hanji",
    summary:
      "한글 문서와 PDF, 이미지, 동영상을 한 자리에서 열고 바꾸는 도구를 만들어 공개했습니다. " +
      "웹과 데스크탑 앱 두 가지로 내놓았습니다.",
    problem:
      "온라인 변환기는 예외 없이 파일을 서버로 보냅니다. 계약서나 인사 서류를 다루는 사람에게 " +
      "'금방 지웁니다'는 답이 되지 않습니다. 게다가 2026년 5월부터 행정기관 전자문서가 개방형 포맷으로 " +
      "의무화되면서 한글 문서를 변환할 일 자체가 늘었습니다.",
    approach:
      "파싱과 렌더링, 인코딩을 전부 브라우저 안에서 끝냈습니다. 그러면 주장이 하나로 좁혀지는데, " +
      "그 하나를 말로 하지 않고 실제 브라우저로 문서를 통과시키며 외부 요청이 0건인지 검사 40개로 " +
      "매번 확인합니다. 데스크탑 앱에는 네트워크 라이브러리를 아예 넣지 않아 의존성 목록 자체가 근거가 됩니다.",
    result:
      "설치 파일 9.3MB로 macOS와 Windows에 배포했고 소스는 MIT로 공개했습니다. " +
      "한글 표가 페이지를 벗어나던 결함은 상위 엔진 358개 커밋을 이분 탐색해 회귀 지점을 찾아 고쳤습니다.",
    metrics: [
      { k: "무업로드 검증", v: "실제 브라우저 검사 40개 · 외부 요청 0" },
      { k: "설치 용량", v: "9.3MB · 네트워크 라이브러리 0개" },
      { k: "상위 엔진 결함", v: "358커밋 이분 탐색 후 패치" }
    ],
    stack: ["TypeScript", "WebAssembly", "Tauri", "Rust", "pdf-lib"],
    tags: ["제품", "오픈소스", "로컬 우선"],
    link: { label: "내려받기 페이지", url: "https://sylvanus4.github.io/hanji-download/" },
    featured: true
  },
  {
    id: "2i-studio",
...
export const contact = {
  title: "다음으로 하고 싶은 일",
  body:
    "회사 전체의 AI 전환을 이끄는 일을 하고 싶습니다. 팀 하나의 성과로는 닿지 않는 범위라서, " +
    "그 권한을 함께 논의할 수 있는 분과 이야기 나누고 싶습니다.",
  /* 이 단락들은 링크를 받은 분이 스스로 판단하도록 쓴 것이다. 설득이 아니라 선별이 목적이라
     맞지 않을 수 있다는 말을 먼저 둔다. */
  stance: [
    "AI 전환을 고민하고 계신 중소기업 대표님이라면 드릴 수 있는 답이 있다고 생각합니다. " +
      "다만 솔직하게 말씀드리면, 회사가 지금 서 있는 자리와 제가 일하는 방식 사이의 간격이 커서 " +
      "서로 맞지 않을 수도 있습니다. 그건 짧게 이야기해 보면 금방 드러납니다.",
    "간격과 무관하게 분명한 것이 하나 있습니다. 회사의 소프트웨어가 경쟁사를 압도하게 만드는 " +
      "일이라면 어렵지 않게 도와드릴 수 있습니다. 기술이나 방법을 따로 증명하는 자리를 만들기보다 " +
      "이 사이트에 쌓여 있는 것을 먼저 보시는 편이 빠릅니다. 여기 있는 것들이 그 답입니다.",
    "그리고 지금은 시간이 중요한 시기라고 봅니다. 같은 결정을 1년 뒤에 내리면 드는 비용이 " +
      "달라집니다. 전사 범위의 전환은 조직과 예산과 우선순위를 함께 움직여야 가능해서 팀장 역할로는 " +
      "할 수 있는 일이 분명히 제한됩니다. 그래서 그 범위를 맡길 수 있는 대표님, 또는 그에 준하는 " +
      "결정권을 가진 분의 연락을 기다리고 있습니다."
  ],
  links: [
...
export const reels = [
  /* lead: 홈과 카탈로그 맨 위에 크게 거는 대표 한 편. 여기 플래그를 옮기면
     두 화면이 같이 따라온다 — 어느 편을 앞세울지는 콘텐츠가 정하지 화면이 정하지 않는다. */
  { n: "00", slug: "00-ainative", cat: "AI 네이티브 · 자비스", palette: "amber", ready: true, dur: "0:50",
    lead: true,
    hook: "말로 시작해서 말로 끝냅니다.",
    blurb: "말로 시작해서 말로 끝냅니다. 스킬 1,930개를 비서 하나가 골라 씁니다. 지금 이 노트북에서 이미 돌고 있는 방식입니다." },
  { n: "01", slug: "01-comms", cat: "통신 · 전 구간", palette: "cyan", ready: true, dur: "0:52",
    blurb: "신호 지문을 전파로 일반화하고, 송신기와 채널과 수신기를 분리해 일반 서버 한 대에서 돌립니다." },
  { n: "02", slug: "02-recsys", cat: "추천 시스템", palette: "amber", ready: true, dur: "1:00",
    blurb: "쿠폰 예산, 배송 물량, 오퍼 랭킹처럼 회사가 실제로 풀고 싶은 문제로 옮겨 놓습니다." },
  { n: "03", slug: "03-agent", cat: "에이전트 플랫폼", palette: "violet", ready: true, dur: "0:53",
    blurb: "스킬 2,000개를 비서 하나가 운영합니다. 라우팅과 루프와 검증을 코드가 소유합니다." },
  { n: "04", slug: "04-nphard", cat: "NP-하드 최적화", palette: "emerald", ready: true, dur: "0:43",
    blurb: "근무표, 콜센터 인력, 리그 일정처럼 딥러닝으로 풀기 어려운 배정 문제를 다룹니다." },
  { n: "05", slug: "05-quant", cat: "LLM 양자화", palette: "indigo", ready: true, dur: "0:48",
    blurb: "같은 모델을 더 싸게 굴립니다. 서빙까지 검증한 것만 셉니다." },
  { n: "06", slug: "06-distill", cat: "소형 모델 증류·이식", palette: "teal", ready: true, dur: "0:31",
    blurb: "반복 워커만 떼어 소형 모델로 옮기고, 판단이 필요한 곳에만 큰 모델을 씁니다." },
  { n: "07", slug: "07-acoustic", cat: "음향", palette: "lime", ready: true, dur: "0:40",
  ...
  { n: "11", slug: "11-marketing", cat: "마케팅 자동화", palette: "amber", ready: true, dur: "0:28",
    blurb: "기술 블로그 무인 운영과 경쟁사 인텔. 발행까지 자동으로 갑니다." },
  { n: "12", slug: "12-storage", cat: "스토리지 내구성", palette: "teal", ready: true, dur: "0:34",
    blurb: "결함을 실제로 주입해 세어 본 것만 남깁니다. 공급자 주장을 실측으로 검증합니다." },
  { n: "13", slug: "13-product", cat: "도메인 제품", palette: "emerald", ready: true, dur: "0:29",
    blurb: "법률, 교육, 의료, 금융. 규제가 있는 곳에서 로컬 우선으로 만듭니다." },
  { n: "14", slug: "14-devtool", cat: "개발 도구", palette: "azure", ready: true, dur: "0:27",
    blurb: "나머지 전부를 빠르게 만드는 바닥. 템플릿, 검사기, 문서화, 저작 도구." },
  { n: "15", slug: "15-career", cat: "커리어 아카이브", palette: "crimson", ready: true, dur: "0:53",
