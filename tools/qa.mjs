...
async function runTech(name, ctxOpts) {
  ...
  cards === 160 ? ok(`카드 ${cards}개`) : bad(`카드 ${cards}개 (기대 160)`);
  ...
  shown > 0 && shown < 160 ? ok(`검색 필터 동작 (${shown}개)`) : bad(`검색 필터 이상 (${shown}개)`);
...
async function techEnGate() {
  ...
  try {
    ...
    titles.length >= 160
      ...
      : bad(`body 제목 ${titles.length}개 — 160 미만, 추출 회귀 의심`);
    missing.length === 0
      ? ok("영문 오버레이 커버리지 전량")
  ...
  r.cards === 160 ? ok(`영문 화면 카드 ${r.cards}개`) : bad(`영문 화면 카드 ${r.cards}개 (기대 160)`);
