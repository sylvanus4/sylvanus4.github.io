...
node tools/audit.mjs              # 접근성 · UX (4개 페이지 x 7개 폭)
...
`audit.mjs` 검사 항목: h1 유일성, heading 단계 건너뜀, main 랜드마크, 이미지 alt, 폼 라벨,
스킵 링크가 첫 Tab 정지점인지, 포커스 표시 유무, **알파 합성 기반 대비**(반투명 배경을 흰색
위에 칠하면 다크 테마 값이 통째로 틀린다), aria-expanded 실제 토글, 폭별 오버플로,
**좁은 화면에서 내비 도달 가능 여부**.
...
- 좁은 화면에서 내비 링크를 `display:none` 으로 숨기면 다른 페이지로 갈 방법이 사라진다.
- 부모에 `backdrop-filter` 가 걸려 있으면 자식의 반투명 배경으로는 본문이 비친다.
- 대비를 잴 때 반투명 배경을 흰색 위에 합성하면 다크 테마에서 전부 오탐이 난다.
